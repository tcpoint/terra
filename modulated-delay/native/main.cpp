...
constexpr size_t MAX_LFO_DELAY = static_cast<size_t>(53400 * 1.f);
...
public:
    ...
    void SetDepth(float _depth) { depth = samplerate * _depth * .040f; }
...
private:
    DelayLine<float, MAX_LFO_DELAY> line;
...
int main(int argc, char *argv[])
{
    ...
    float       feedback = 0.3f;
    float       mix      = 0.5f;
    float       lfoFreq  = 0.3f;  // 0-1
    float       lfoDepth = 0.7f;  // 0-1
    int         waveform = Oscillator::WAVE_TRI;
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        else
        {
            fprintf(stderr,
                    ...
                    "[--mix <0-1>] [--lfo-freq <0-1>] [--lfo-depth <0-1>] "
                    "[--waveform <0-7>]\n",
                    argv[0]);
            return 1;
        }
    ...
    state.delay.SetDelay(delayMs * 0.001f * sampleRate);
    state.delay.feedback = feedback;
    state.delay.SetFreq(lfoFreq);
    state.delay.SetDepth(lfoDepth);
    state.delay.SetWaveform(waveform);
    state.cfade.Init();
    ...
    if(state.wavMode)
    {
        printf("Playing '%s' through modulated-delay (delay=%.0fms feedback=%.2f "
               "mix=%.2f lfo-freq=%.2f lfo-depth=%.2f waveform=%d)...\n",
               wavPath, delayMs, feedback, mix, lfoFreq, lfoDepth, waveform);
    ...
    else
    {
        ...
        printf("Live mode (delay=%.0fms feedback=%.2f mix=%.2f lfo-freq=%.2f "
               "lfo-depth=%.2f waveform=%d). Press Ctrl+C to stop.\n",
               delayMs, feedback, mix, lfoFreq, lfoDepth, waveform);
        while(g_keepRunning)
        {
            Pa_Sleep(100);
        }
