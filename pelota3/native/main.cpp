...
struct Delay
{
    ...
    float Process(float in)
    {
        ...
        line.Write(((feedback * read) + filtered_input) * 0.5f);
...
int main(int argc, char *argv[])
{
    ...
    state.delay.lpf_in.Init(static_cast<float>(sampleRate));
    state.delay.lpf_in.SetFreq(5400.0f);
    state.delay.lpf_in.SetQ(0.76f);
    state.delay.lpf_out1.Init(static_cast<float>(sampleRate));
    state.delay.lpf_out1.SetFreq(2200.0f);
    state.delay.lpf_out1.SetQ(0.8f);
    state.delay.lpf_out2.Init(static_cast<float>(sampleRate));
    state.delay.lpf_out2.SetFreq(3300.0f);
    ...
    if(state.wavMode)
    {
        printf("Playing '%s' through pelota3 (delay=%.0fms feedback=%.2f mix=%.2f)...\n",
