  ...
  private:
	...
	public:
		...
		void setparams (TSParameters & p)
			{
				float R1 = p.R1, R2 = p.R2, R3 = p.R3, R4 = p.R4;
				float C1 = p.C1, C2 = p.C2, C3 = p.C3;
		...
		inline void updatecoefs (float b, float m, float t)
			{
				struct {
						float a1, a2, a3;
						float b1, b2, b3;
				...
				float dcoef_a[Order + 1];
				float dcoef_b[Order + 1];
