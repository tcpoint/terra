...
constexpr size_t MAX_DELAY = static_cast<size_t>(48000 * 0.3f);
...
struct Delay
{
    ...
    float Process(float in)
    {
        ...
        line.Write(in + feedback * degraded);
        return degraded;
    }
};
...
int main(int argc, char *argv[])
{
    ...
    float       delayMs  = 150.0f;  // MAX_DELAY caps at ~300ms
    float       feedback = 0.4f;
    float       mix      = 0.5f;
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        {
            fprintf(stderr,
                    "Usage: %s [--wav <path>] [--delay <ms, max ~300>] "
                    "[--feedback <0-1>] [--mix <0-1>]\n",
                    argv[0]);
            return 1;
        }
    }
    ...
    state.delay.tone.Init();
    state.delay.tone.SetFrequency(7000.0f / sampleRate);
    ...
    if(state.wavMode)
    {
        printf("Playing '%s' through burning-delay (delay=%.0fms feedback=%.2f mix=%.2f)...\n",
