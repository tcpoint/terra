                          ...
                          void *userData)
{
    ...
    for(unsigned long i = 0; i < framesPerBuffer; i++)
    {
        ...
        state->ch.Process(sample);
        float wet   = (state->ch.GetLeft() + state->ch.GetRight()) / 2.0f;
...
int main(int argc, char *argv[])
{
    ...
    float       speed    = 0.3f;  // 0-1, mapped to speed*speed*20Hz internally
    ...
    if(state.wavMode)
    {
        printf("Playing '%s' through chorus (delay=%.2f speed=%.2f depth=%.2f "
               "mix=%.2f feedback=%.2f)...\n",
               wavPath, delay, speed, depth, mix, feedback);
    ...
    else
    {
        ...
        printf("Live mode (delay=%.2f speed=%.2f depth=%.2f mix=%.2f "
               "feedback=%.2f). Press Ctrl+C to stop.\n",
               delay, speed, depth, mix, feedback);
        while(g_keepRunning)
        {
            Pa_Sleep(100);
        }
